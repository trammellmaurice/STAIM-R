...
const NUM_INPUTS = 5; //FIGHTER x, y, z, and heading + TARGET x, y, and z
const NUM_HIDDEN = 35; // IDEA: Probably more hidden layers than the AA Gun?
const NUM_OUTPUTS = 2; //FIGHTER thrust, yaw, climb
...
if(AUTOMATION_ON)
{
  ...
  nnAlt = new NeuralNetwork(2, 10, 1);
  let fx, fy, tx, ty;
  for (let i = 0; i < NUM_SAMPLES; i++){
    ...
    nnAlt.train([tz/MAX_ALT,fz/MAX_ALT], [alt])
  }
}
/**********************************************
...
function updateEnvironment(){
  ...
  if(fighter.alive || DEV) //fighter alive or developer mode
  {
    if(AUTOMATION_ON)
    {
      ...
      let predictAlt = nnAlt.feedForward([tz,fz]).data[0];
      console.log(predictAlt);
      ...
      let climb = Math.abs(predictAlt[0] - CLIMB);
      let drop = Math.abs(predictAlt[0] - DROP);
      ...
      if(climb < OUTPUT_THRESHOLD){fighter.climb();} //ascend
      else if(drop < OUTPUT_THRESHOLD){fighter.drop();} //descend
    }else{
    ...
    if(environment.keys && environment.keys[187]){target.raise();}
    if(environment.keys && environment.keys[189]){target.lower();}
  }
